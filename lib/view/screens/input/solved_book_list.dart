...
class SolvedBookList extends StatelessWidget {
  final UserModel userModel;
  final setNumOfSolvedBooks;
  const SolvedBookList({Key? key, required this.userModel, this.setNumOfSolvedBooks}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    var bookKeys = [];
    for (int i = 0; i < userModel.solvedBooksAndScore.length; i++) {
      bookKeys.add(userModel.solvedBooksAndScore[i].bookKey);
    }
    final userKey = FirebaseAuth.instance.currentUser!.uid;
    return FutureBuilder<List<BookModel>>(
        future: BookService().getBooksByBookKeys(bookKeys),
        builder: (context, snapshot) {
          if (snapshot.hasData && snapshot.data!.isNotEmpty) {
            List<BookModel> _myBooks = snapshot.data!;
            setNumOfSolvedBooks(_myBooks.length);
